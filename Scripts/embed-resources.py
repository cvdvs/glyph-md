...
def main():
    ...
    # Every inline block must carry the nonce placeholder. A new <script> added
    # without one is not a syntax error and not a warning - the CSP simply refuses
    # to run it, and the page half-works. Blocks live at column 0; the matches
    # further in are string literals inside guardInput. (CLAUDE.md rule 18.)
    for tag in (b"script", b"style"):
        opened = len(re.findall(rb"(?m)^<" + tag + rb"[ >]", viewer))
        nonced = len(re.findall(rb"(?m)^<" + tag + rb' nonce="/\*__CSP_NONCE__\*/">', viewer))
        if opened != nonced:
            print(f"FAIL  Resources/viewer.html has {opened} inline <{tag.decode()}> blocks but "
                  f"{nonced} carry nonce=\"/*__CSP_NONCE__*/\".\n"
                  "      An un-nonced block is silently blocked by the CSP. Rule 18: the\n"
                  "      nonce path (macOS + the shell) and the sha256 path\n"
                  "      (make-preview.py, engine-smoke.mjs) must be updated together.",
                  file=sys.stderr)
            return 1
    print(f"  placeholders ok: {len(PLACEHOLDERS)} substitution points, "
          f"{len(re.findall(rb'(?m)^<(script|style)[ >]', viewer))} inline blocks all nonced")
